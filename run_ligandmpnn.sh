...
BASE_DIR=/home/sona/LigandMPNN
INPUT_PATH=/home/sona/RFdiffusion/outputs/modified
OUTPUT_DIR=$BASE_DIR/outputs/modified

CACHE_FILE="$INPUT_PATH/cache.txt"

# Check if cache file exists
if [ -f "$CACHE_FILE" ]; then
    # Read the cache file and store the processed filenames in an array
    declare -a PROCESSED_FILES=()
    while IFS= read -r line; do
        PROCESSED_FILES+=("$line")
    done < "$CACHE_FILE"
else
    # Create an empty array if cache file doesn't exist
    declare -a PROCESSED_FILES=()
fi

# Iterate over all pdb files in the input directory
for file in "$INPUT_PATH"/*.pdb; do
    ...
    if [[ " ${PROCESSED_FILES[@]} " =~ " ${file} " ]]; then
        ...
        # Add your existing code here to process the file
        python $BASE_DIR/run.py \
            --seed $RANDOM_SEED \
            --pdb_path "$file" \
            --out_folder "$OUTPUT_DIR" \
            --model_type "protein_mpnn" \
            --checkpoint_protein_mpnn "$BASE_DIR/model_params/proteinmpnn_v_48_002.pt" \
            --temperature 0.01 \
            --zero_indexed 1 \
            --number_of_batches 100 \
            --batch_size 1 \
            --chains_to_design "A" 
        # Append the processed filename to the cache file
